...
[Spline](https://spline.design) is a collaborative design platform for creating production-ready interactive experiences in multiple dimensions. Design 3D and 2D scenes, animations and interactions in the browser, then export them to the web and native apps — or generate them with AI.

**@splinetool/runtime** is the JavaScript runtime that plays those scenes: it loads a `.splinecode` file exported from the Spline editor and renders it on a `<canvas>` (WebGPU, with a WebGL fallback), with full access to the scene's objects, events and variables.

[Website](https://spline.design) — [Docs](https://docs.spline.design) — [Community](https://community.spline.design) — [Blog](https://blog.spline.design) — [Updates](https://updates.spline.design)
