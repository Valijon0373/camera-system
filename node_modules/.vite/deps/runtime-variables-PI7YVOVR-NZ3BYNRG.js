...
// node_modules/@splinetool/runtime/build/runtime-variables-PI7YVOVR.js
var b = new As();
var v = new Ki();
var f = new Qi();
var g = new Ki();
  ...
  constructor(a2, t2, i, s2, r) {
    super(a2), this.sharedVariables = t2, this.eventManager = i, this.needsMouse = s2, this.needsRaycast = r, this.dynamicVars = [], this.variablesToWatch = [], this.prevMouseProperty = null, this.prevRaycastProperty = null, this.pauseTime = 0, this.isOutOfFocus = false, this.events = [], this.onFrame = (a3) => {
      ...
      for (let r3 = 0; r3 < this.dynamicVars.length; r3++) {
        let { id: n3, expectedTime: d2, last: o, data: c, timerSeconds: u2, startValue: p } = this.dynamicVars[r3], y = 1e3;
        if ("timer" !== c.dynamicVariableType && "stopwatch" !== c.dynamicVariableType || "number3decimal" !== c.format ? "counter" === c.dynamicVariableType ? y = c.updateInterval : "random" === c.dynamicVariableType && (y = c.isStatic ? 1 / 0 : c.updateInterval) : y = 1, -1 !== d2) {
          if ("Stopped" !== t3.getDynamicVariablePlayState(n3) && "Restarted" !== t3.getDynamicVariablePlayState(n3) || (this.dynamicVars[r3].last = a3, this.dynamicVars[r3].expectedTime = a3 + y, "counter" === c.dynamicVariableType && t3.getVariable(n3) !== p && (t3.updateVariable(n3, p), this.eventManager.requestRender()), ("timer" === c.dynamicVariableType || "stopwatch" === c.dynamicVariableType) && this.dynamicVars[r3].timerSeconds !== p && (this.dynamicVars[r3].timerSeconds = p, t3.updateVariable(n3, wa.getFormattedTimerTime(p, c)), this.eventManager.requestRender()), "Restarted" === t3.getDynamicVariablePlayState(n3) && t3.setDynamicVariablePlayState(n3, "Playing"), "Stopped" !== t3.getDynamicVariablePlayState(n3))) {
            ...
            if (this.dynamicVars[r3].deltaToExpectedTime = void 0, a3 >= d2) if (this.dynamicVars[r3].last = a3, this.dynamicVars[r3].expectedTime += y, "random" === c.dynamicVariableType) {
              ...
              c.hasEnd && (i3 = c.increment > 0 ? Zi.clamp(i3, p, c.endValue) : Zi.clamp(i3, c.endValue, p), i3 === p && t3.setDynamicVariableToggleIsForward(n3, void 0), c.repeat ? i3 === c.endValue && (i3 = p) : (i3 === p || i3 === c.endValue) && t3.setDynamicVariablePlayState(n3, "Paused")), i3 !== e2 && (s3.push(n3), t3.updateVariable(n3, i3), this.eventManager.requestRender());
              ...
              let e2 = ya.isTextValue(c.endValue) ? w2(c.endValue.textValue) : c.endValue, i3 = t3.getDynamicVariableToggleIsForward(n3) ?? true, d3 = u2 + (a3 - o) / 1e3 * ("timer" === c.dynamicVariableType ? -1 : 1) * (i3 ? 1 : -1);
              d3 = Math.max(d3, 0), c.hasEnd && (d3 = "stopwatch" === c.dynamicVariableType ? Zi.clamp(d3, p, e2) : Zi.clamp(d3, e2, p), d3 === p && t3.setDynamicVariableToggleIsForward(n3, void 0), c.repeat ? Math.abs(d3 - e2) < ("number3decimal" === c.format ? 1e-4 : 0.1) && (d3 = p) : (Math.abs(d3 - p) < ("number3decimal" === c.format ? 1e-4 : 0.1) || Math.abs(d3 - e2) < ("number3decimal" === c.format ? 1e-4 : 0.1)) && t3.setDynamicVariablePlayState(n3, "Paused")), d3 !== u2 && (s3.push(n3), this.dynamicVars[r3].timerSeconds = d3, t3.updateVariable(n3, wa.getFormattedTimerTime(d3, c)), this.eventManager.requestRender());
            }
          }
...
//# sourceMappingURL=runtime-variables-PI7YVOVR-NZ3BYNRG.js.map
