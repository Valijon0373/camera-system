...
// node_modules/@splinetool/runtime/build/runtime-chunk-42DQ6LLX.js
var A = 200;
var W = null;
function O2(t3) {
  W = t3;
...
function F(t3) {
  ...
  return t3.scene.objects.forEach((i) => {
    "Page" === i.data.type && !e3 && i.data.publish.gameControlObject && ua.traverseModuleInstances(t3, i.id, (t4, i2, s3) => {
...
function lt(t3) {
  return rt || (rt = async function() {
    let e3 = t3 ?? "https://cdn.spline.design/@splinetool/runtime@2.0.53/build", i = import("./physics-XSCNPI7S.js"), [s3, o3] = await Promise.all([i, fetch(`${e3}/physics.wasm`).then((t4) => t4.arrayBuffer())]);
...
var mt2 = class {
  ...
  dispatch(t3) {
    this.initialDisabled || (this.actions.Transition.forEach((t4) => {
      ...
      "Toggle" === t4.data.runMode ? t4.toggle() : t4.play();
...
function Lt(t3) {
  if (ua.physicsEnabled(t3.scene.objects)) return true;
  let e3 = false;
  ...
  rebuildBVH() {
    this.usePhysics ? this.initPhysics() : (this.page.traverseEntity((t3) => {
      for (let e3 of t3.dataPatched.events) if ("Trigger" === e3.data.type && true !== e3.data.disabled) {
        let i = new ut(e3.id, e3.data, t3, this.page, this.sharedAssets, this.eventManager), s3 = new As().compose(bt.fromArray(e3.data.position.map((t4) => "number" == typeof t4 ? t4 : 0)), Pt.setFromEuler(ft.fromArray(e3.data.rotation.map((t4) => "number" == typeof t4 ? t4 : 0))), Ct);
  ...
  generateSensorColliderDescs(t3, e3, i) {
    ...
    t3.traverseEntity((o3, a2) => {
      ...
      for (let e4 of o3.dataPatched.events) if ("Trigger" === e4.data.type && true !== e4.data.disabled) {
        ...
        if (a3 = "box" === e4.data.triggerZone ? pt.ColliderDesc.cuboid(...e4.data.size.map((t4) => ("number" == typeof t4 ? t4 : 1) / (2 * this.pixelsPerMeter))) : pt.ColliderDesc.ball(e4.data.radius / this.pixelsPerMeter), a3.setDensity(0).setSensor(true).setActiveEvents(pt.ActiveEvents.COLLISION_EVENTS), bt.fromArray(e4.data.position.map((t4) => "number" == typeof t4 ? t4 : 0)), Pt.setFromEuler(ft.fromArray(e4.data.rotation.map((t4) => "number" == typeof t4 ? t4 : 0))), Mt.compose(bt, Pt, Ct).premultiply(o3.matrixWorld), i ? (Mt.decompose(bt, Pt, Et), a3.setActiveCollisionTypes(pt.ActiveCollisionTypes.KINEMATIC_FIXED | pt.ActiveCollisionTypes.DYNAMIC_FIXED)) : (Mt.premultiply(s3).decompose(bt, Pt, Et), t3.rigidBody.bodyType() === pt.RigidBodyType.Dynamic || (t3.rigidBody.bodyType() === pt.RigidBodyType.Fixed ? a3.setActiveCollisionTypes(pt.ActiveCollisionTypes.KINEMATIC_FIXED | pt.ActiveCollisionTypes.DYNAMIC_FIXED) : a3.setActiveCollisionTypes(pt.ActiveCollisionTypes.KINEMATIC_KINEMATIC | pt.ActiveCollisionTypes.DYNAMIC_KINEMATIC))), bt.divideScalar(this.pixelsPerMeter), a3.setTranslation(bt.x, bt.y, bt.z), a3.setRotation(Pt), i) i.push([a3, n4]);
        else {
  ...
  checkTrigger() {
    ...
    for (let e3 = 0; e3 < this.sharedGameControlGlobals.triggers.length; e3++) {
      ...
      for (let e4 = 0; e4 < this.sharedGameControlGlobals.entitiesWithTransformAnim.length; e4++) {
        ...
        let r3 = xt.copy(o3.matrixWorld).decompose(bt, St, Et).compose(bt, St, Ct).multiply(s3), h = Mt.copy(n4.matrixWorld).invert().multiply(r3), l = n4.bvhGeometry.boundsTree;
...
function kt(t3) {
  return Ot || (Ot = async function() {
    let e3 = t3 ?? "https://cdn.spline.design/@splinetool/runtime@2.0.53/build";
    typeof window < "u" && (Wt = new URL(e3.endsWith("/") ? e3 : `${e3}/`, window.location.href).href);
    let i = import("./navmesh-OA2WFIGN.js"), [s3, o3] = await Promise.all([i, fetch(`${e3}/navmesh.wasm`).then((t4) => t4.arrayBuffer())]), a2 = s3.default, n4 = await a2({ wasmBinary: o3 });
    At(n4);
  }());
}
...
//# sourceMappingURL=chunk-IUNMUO77.js.map
