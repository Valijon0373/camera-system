...
// node_modules/@splinetool/runtime/build/runtime-chunk-5QJOECK2.js
...
var I = (t, e, i) => {
  t.updateEntityBoxSize(O, W), _.copy(e).multiply(t.matrixWorld), 0 === W.x && 0 === W.y && 0 === W.z ? i.push(new Ki(O.x, O.y, O.z).applyMatrix4(_)) : R.forEach((t2) => {
    i.push(t2.clone().multiply(W).add(O).applyMatrix4(_));
  });
};
var z = class extends wr {
  ...
  computeVertices() {
    this.getSize(W).multiplyScalar(0.5), this.getCenter(O), _.copy(this.matrix).setPosition(O), this.vertices = R.map((t) => t.clone().multiply(W).applyMatrix4(_));
  }
  computeEdges() {
...
function Z(t) {
  let e = t.elements;
  L.set(e[0], e[1], e[2]), F.set(e[4], e[5], e[6]), N.set(e[8], e[9], e[10]);
}
function Q() {
  let t = Math.max(L.lengthSq(), F.lengthSq(), N.lengthSq());
  L.lengthSq() > H * t ? j.copy(L).normalize() : j.set(1, 0, 0), k.crossVectors(N, j), k.lengthSq() <= H * t && (k.copy(F).addScaledVector(j, -F.dot(j)), k.lengthSq() <= H * t && function(t2, e) {
    t2.set(1, 0, 0).cross(e), t2.lengthSq() <= H && t2.set(0, 1, 0).cross(e), t2.normalize();
  }(k, j)), k.normalize(), q.crossVectors(j, k);
}
function $(t) {
  return Z(t), Q(), G.makeBasis(j, k, q);
}
var K;
var Y2 = new As();
(K || (K = {})).is = function(t) {
...
var J = (t) => class extends t {
  ...
  getPhysicsLocalMatrix(t2) {
    var _a;
    return !!((_a = this.rigidBody) == null ? void 0 : _a.isDynamic()) && (t2.copy(this.matrixWorld), this.hasNonUniformScale && t2.multiply(this.shearScaleInv), this.parent && t2.premultiply(Y2.copy(this.parent.matrixWorld).invert()), t2.premultiply(Y2.copy(this.hiddenMatrix).invert()), t2.multiply(Y2.copy(this.pivotMatrix).invert()), true);
  }
  isDescendantOf(t2) {
  ...
  traverseChildren(t2, e = 0) {
    for (let i of this.children) K.is(i) && i.traverseObject(t2, e + 1);
  }
  traverseObject(t2, e = 0) {
    if (true !== t2(this, e)) for (let i of this.children) K.is(i) && i.traverseObject(t2, e + 1);
  }
  updateMatrixWorldRigid() {
    this.hasNonUniformScale = function(t2, e, i) {
      return Z(t2), Q(), e.makeBasis(j, k, q), i.set(j.dot(L), j.dot(F), j.dot(N), 0, k.dot(L), k.dot(F), k.dot(N), 0, q.dot(L), q.dot(F), q.dot(N), 0, 0, 0, 0, 1), function() {
        let t3 = L.length(), e2 = F.length(), i2 = N.length(), r = Math.max(t3, e2, i2);
        if (r - t3 > X || r - e2 > X || r - i2 > X) return true;
        let s2 = X * r * r;
        return Math.abs(L.dot(F)) > s2 || Math.abs(L.dot(N)) > s2 || Math.abs(F.dot(N)) > s2;
      }();
...
var Mt = (t) => mt2.is(t);
var wt = { type: "completeState", isfromEntity: true };
var bt = ["x", "y", "z"];
var Bt = new Ki();
var Pt = new Ki(1, 1, 1);
var Et = new Qi();
...
var Dt = (c) => class extends J(c) {
  ...
  resetBBoxNeedsUpdate() {
    this.resetBBoxNeedsUpdateSelf(), this.traverseAncestors((t) => {
      Mt(t) && t.resetBBoxNeedsUpdateSelf();
  ...
  forEachEntity(t) {
    for (let e of this.children) Mt(e) && t(e);
  }
  ...
  traverseConcreteEntity(t, e = 0) {
    if (true !== t(this, e)) for (let i of this.children) Mt(i) && i.isConcreteEntity && i.traverseEntity(t, e + 1);
  }
  traverseEntity(t, e = 0) {
    if (true !== t(this, e)) for (let i of this.children) Mt(i) && i.traverseEntity(t, e + 1);
  }
  traverseVisibleEntity(t) {
    t(this);
    for (let e of this.children) Mt(e) && e.visible && e.traverseVisibleEntity(t);
  }
  updateMatrix() {
  ...
  hasEntityChild() {
    return this.children.some((t) => Mt(t));
  }
  isAncestorOf(t) {
  ...
  updateByOp(t, e, i, s2) {
    ...
    if (null !== c2) {
      if (0 === t.type) {
        ...
        if ((this == null ? void 0 : this.stateSelection) === e2) {
          ...
          l = { ...t, props: e3, path: t.path.slice(2) };
        }
      }
    } else if (0 === t.type) {
    ...
    if (this.updateByPatchedOpBase(l, ia.patch(this.data, this.stateSelection ? this.data.states.data(this.stateSelection) : void 0), i), V(t.path, ["overrides"])) {
      let r = [], s3 = [...t.path];
      for (r.push(s3[1]), s3.splice(0, 2); s3.length > 0 && "descendants" === s3[0]; ) r.push(s3[1]), s3.splice(0, 2);
      if (void 0 === r[r.length - 1]) {
        if (0 === t.type) for (let e2 of Object.keys(t.props)) {
          r[r.length - 1] = e2;
          let s4 = i.scene.findInstance([this.uuid, ...r]);
          s4 && (s4.overrideData = t.props[e2], s4.updateState(M.apply(s4.component.data, s4.overrideData), i));
        }
      } else {
        let a2 = i.scene.findInstance([this.uuid, ...r]);
        if (a2) {
          let n2 = Y.zoom(a2.component.data, s3);
          if (0 === (t = { ...t, path: s3 }).type) {
            let e2 = t.props;
            if (n2) for (let [i2, r2] of Object.entries(t.props)) void 0 === r2 && (e2 === t.props && (e2 = { ...t.props }), e2[i2] = n2[i2]);
            t = { ...t, props: e2 };
          }
          a2.overrideData = ea.resolve(e.overrides, r), a2.updateByOp(t, E.applySimple(a2.data, t), i, false);
        }
      }
      ...
      if (0 === t.path.length && 0 === t.type) {
        ...
        e2 && (s3 = { ...t, props: e2 });
      } else for (let e2 of ta.rootOverrideProps) if (V(t.path, [e2])) {
        s3 = t;
        break;
      }
      void 0 !== s3 && this.instances.forEach((t2) => {
        if (t2.isInstanceRoot) {
          let e2 = M.filterOp(t2.overrideData, s3);
          e2 && t2.updateByOp(e2, E.applySimple(t2.data, e2), i, true);
        }
      }), this.instances.forEach((r) => {
        if (!r.isInstanceRoot) {
          let s4 = M.filterOp(r.overrideData, t);
          if (s4) {
            let a2;
            a2 = n === r.data && t === s4 ? e : E.applySimple(r.data, s4), r.updateByOp(s4, a2, i, true);
          }
        }
      });
    }
  ...
  updateByPatchedOp(t, e, i) {
    ...
    if (null !== V(t.path, ["pathSnapping"]) && this.updatePathSnapping(e.pathSnapping), null !== V(t.path, ["cloner"])) {
      let r = E.drop(t, 1);
  ...
  applyPathSnapping(t) {
    ...
    this.updateTransformState({ position: d.position.toArray(), rotation: d.rotation.toArray() }), this.traverseEntity((t2) => {
      t2._cameraType && t2.dispatchEvent(wt);
    });
  }
  updateVisible(t) {
    var _a;
...
var Tt = class extends ha {
  constructor(t = true, e = 1, i = 1, r = 1, s2 = 1, a2 = 1, n = 1, o2 = 1, l = 1, h = 1, p = 1, d = 1) {
    ...
    let u, m, y, g, v, x, S, M2, w = () => new Ki(), B = new Ki(), P = w(), E2 = w(), A = w(), C2 = w(), D2 = w(), V2 = w(), O2 = w(), W2 = w(), _2 = w(), R2 = w(), T3 = w(), U2 = i - 2 * o2 + 1e-3, I2 = U2 / a2, z2 = Math.ceil(n * a2), L2 = z2 + 1, F2 = U2 / z2, N2 = -U2 / 2, j2 = h + 1, k2 = 2 * Math.PI / h, q2 = Math.PI / 2 / d, G2 = Math.min((1 - p / 100) * o2, o2 - 0.01), H2 = o2 - G2, X2 = 0, Z2 = 2 * d + 2, Q2 = j2 * Z2 / 2, $2 = Q2 + j2 * L2, K2 = Math.max(0, j2 * (L2 + Z2)), [Y3, J2, tt2] = [3, 3, 2].map((t2) => Array(K2 * t2).fill(0)), et2 = [], it2 = s2 - o2;
    function rt2(e2, i2) {
      let r2 = Math.PI / 2;
      x = i2 * F2, M2 = 2 * Math.PI * (x % I2) / I2 + r2, x += N2, S = Math.sin(M2) * it2, v = Math.cos(M2) * it2, t ? e2.set(v, S, x) : e2.set(v, x, S);
    }
    ...
    for (let e2 = 0; e2 <= z2; e2++) {
      ...
      let i2 = 0 === e2, r2 = e2 === z2, s3 = i2 ? 3 * Math.PI / 2 : q2, a3 = i2 ? ot2 : lt2, n2 = i2 ? j2 : $2, l2 = i2 ? 0 : K2 - j2, p2 = T3.clone().multiplyScalar(i2 ? -H2 : H2).add(E2), u2 = T3.clone().multiplyScalar(i2 ? -1 : 1).normalize();
      for (let t2 = 0; t2 < j2; t2++) {
        let m2 = t2 * k2;
        if (D2.addVectors(B.copy(_2).multiplyScalar(o2 * Math.cos(m2)), P.copy(R2).multiplyScalar(o2 * Math.sin(m2))), V2.copy(D2).normalize(), i2 || r2) {
          c || (X2 = l2 + t2, [0, 1, 2].forEach((t3) => {
            Y3[3 * X2 + t3] = p2.getComponent(t3), J2[3 * X2 + t3] = u2.getComponent(t3);
          }), tt2[2 * X2] = +r2, tt2[2 * X2 + 1] = t2 / h), P.copy(V2).multiplyScalar(G2), A.addVectors(E2, P);
          for (let e3 = 0; e3 < d; e3++) {
            let r3 = e3 * q2 + s3;
            O2.addVectors(B.copy(T3).multiplyScalar(H2 * Math.sin(r3)), P.copy(V2).multiplyScalar(H2 * Math.cos(r3))), W2.copy(O2).normalize(), P.addVectors(A, O2), O2.normalize(), X2 = n2 + e3 * j2 + t2, [0, 1, 2].forEach((t3) => {
              Y3[3 * X2 + t3] = P.getComponent(t3), J2[3 * X2 + t3] = W2.getComponent(t3);
            });
            let o3 = +i2 + Math.sin(r3);
            tt2[2 * X2] = (a3 + H2 * o3) / nt3, tt2[2 * X2 + 1] = t2 / h;
          }
        }
        P.addVectors(E2, D2), X2 = Q2 + e2 * j2 + t2, [0, 1, 2].forEach((t3) => {
          Y3[3 * X2 + t3] = P.getComponent(t3), J2[3 * X2 + t3] = V2.getComponent(t3);
        }), tt2[2 * X2] = (at2 + e2 * st2) / nt3, tt2[2 * X2 + 1] = t2 / h;
      }
    }
    let ht2 = L2 + 2 * d + 2, [pt2, dt2] = [+c, ht2 - 1];
    ...
    this.setIndex(et2), this.setAttribute("position", new Hr(Y3, 3)), this.setAttribute("normal", new Hr(J2, 3)), this.setAttribute("uv", new Hr(tt2, 2));
  }
  getClosedTorusIndicesForBooleanOrSubdiv() {
    ...
    return Ot.array = new Uint32Array(l), Ot.count = l.length, Ot;
  }
};
...
function zt(t) {
  return Rt || (Rt = async function() {
    let e = t ?? "https://cdn.spline.design/@splinetool/runtime@2.0.53/build", i = import("./boolean-NZSMES6U.js"), [r, s2] = await Promise.all([i, fetch(`${e}/boolean.wasm`).then((t2) => t2.arrayBuffer())]);
...
function Gt(t, e, i, r) {
  let s2 = Lt(), a2 = qt(t, e), n = s2.mesh_render_data(a2, Math.max(r, 0));
...
function Ht(t, e) {
  let i = Lt(), r = qt(t, e), s2 = i.mesh_topological_data(r), a2 = s2.positions(), n = s2.indices();
...
function Qt(t, e) {
  return Nt(Lt().instanced_union(Ft(t), e));
}
function $t(t) {
  return null !== t && "booleanOp" in t;
}
var Kt = class extends Dt(Za) {
  constructor() {
    super(...arguments), this.booleanMeshSetAddress = -1, this.booleanWasTransformed = false, this.booleanMatrixInvOld = new As();
  }
  updateVisible(t) {
    super.updateVisible(t), this.visible = !$t(this.parent) && this.visible, $t(this.parent) && this.parent.invalidateDownstreamBooleanData();
  }
  onClonerLayoutUpdated() {
    $t(this.parent) && this.invalidateDownstreamBooleanData();
  }
  freeBooleanPointer() {
  ...
  invalidateDownstreamBooleanData(t = false) {
    return t ? this.booleanWasTransformed = true : this.freeBooleanPointer(), $t(this.parent) ? this.parent.invalidateDownstreamBooleanData() : this;
  }
  invalidateUpstreamBooleanData() {
    this.freeBooleanPointer();
    for (let t of this.children) t instanceof Kt && (t.freeBooleanPointer(), $t(t) && t.invalidateUpstreamBooleanData());
  }
  updateTransformState(t, e) {
    let i = super.updateTransformState(t, e);
    return i && $t(this.parent) && this.invalidateDownstreamBooleanData(true), i;
  }
  onVariableUpdate(t = false) {
    super.onVariableUpdate(t), $t(this.parent) && this.invalidateDownstreamBooleanData(true);
  }
};
var Yt = new wr();
function Jt(t, e = 0, i = t.count, r, s2) {
  ...
  Yt.min.set(a2, n, o2), Yt.max.set(l, h, p), Yt.getCenter(r), Yt.getSize(s2).multiplyScalar(0.5);
}
var te = new ha();
var ee = new Va();
var ie = class extends Kt {
  ...
  updateEntityBoxSize(t, e) {
    ...
    void 0 !== i ? Jt(i, this.geometry.drawRange.start, this.geometry.drawRange.count < 1 / 0 ? this.geometry.drawRange.count : i.count, t, e) : super.updateEntityBoxSize(t, e);
  }
};
...
//# sourceMappingURL=chunk-FWE3FCUO.js.map
