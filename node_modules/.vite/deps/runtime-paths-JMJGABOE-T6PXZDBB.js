...
// node_modules/@splinetool/runtime/build/runtime-paths-JMJGABOE.js
var A = class extends cr {
  constructor(e, t, s) {
  ...
  updateEntityBoxSize(e, t) {
    ...
    void 0 !== s && s.count > 0 ? Jt(s, this.geometry.drawRange.start, this.geometry.drawRange.count < 1 / 0 ? this.geometry.drawRange.count : s.count, e, t) : super.updateEntityBoxSize(e, t);
  }
};
var D = new As();
var I = new As();
...
var N = class extends ha {
  ...
  build() {
    ...
    let S, { regions: _, infos: M, vertices: A2 } = this._getShapePoints(w), O2 = 0;
    if ("round" === v2) {
      let e2 = this.inputs.shapeData, t2 = G.get(e2);
      void 0 !== t2 && t2.bevel === y && t2.curveSegments === w && t2.bevelSides === f ? S = t2.geometry : (S = new Lt(e2, 2 * y, y, w, f, void 0, true), G.set(e2, { bevel: y, curveSegments: w, bevelSides: f, geometry: S })), O2 = S.getAttribute("position").count;
    }
    let k2 = 0, R2 = 0;
    M.sort((e2, t2) => e2.start - t2.start), M.forEach((e2) => {
      e2.verticesStart = k2, e2.verticesCount = e2.continuous.reduce((e3, t2, s2) => e3 + (0 !== s2 && t2 ? 1 : 2), 0), R2 += e2.verticesCount, k2 = R2;
    });
    let T2, E2 = R2 * o, z2 = 0;
    this._isOpenEnded() && "flat" === v2 && (T2 = this._getSurfaceTesselation(_), z2 = T2.vertexCount);
    let j2 = E2 + 2 * z2 + 2 * O2, B2 = E2 + 2 * z2, U2 = { positions: new Float32Array(3 * j2), normals: new Float32Array(3 * j2), uvs: new Float32Array(2 * j2) }, F2 = S ? S.getIndex().array : null, N2 = 0;
    M.forEach((e2) => {
      N2 += e2.count;
    });
    let L = N2 * (o - 1) * 6 + (T2 ? 2 * T2.elementCount * 3 : 0) + (F2 ? 2 * F2.length : 0), q = new Uint32Array(L), H = 0;
    if (M.forEach((e2) => {
      H = this._extrudeRegion(e2, A2, u, l, U2, q, H, this._isGeometryClosed() && !this._isOpenEnded());
    }), T2 && (H = this._closeEnd(T2, E2, q, H, U2, u[0], l[0], false), H = this._closeEnd(T2, E2 + z2, q, H, U2, u[u.length - 1], l[l.length - 1], true)), S) {
      U2.positions.set(S.getAttribute("position").array, 3 * B2), U2.normals.set(S.getAttribute("normal").array, 3 * B2), U2.uvs.set(S.getAttribute("uv").array, 2 * B2);
      for (let e3 = B2; e3 < B2 + O2; e3++) U2.uvs[2 * e3 + 1] = 1e-4;
      let e2 = H;
      for (let e3 = 0; e3 < F2.length; e3++) q[H++] = F2[e3] + B2;
      B2 += O2, U2.positions.set(S.getAttribute("position").array, 3 * B2), U2.normals.set(S.getAttribute("normal").array, 3 * B2), U2.uvs.set(S.getAttribute("uv").array, 2 * B2);
      let t2 = H;
      for (let e3 = 0; e3 < F2.length; e3++) q[H++] = F2[e3] + B2;
      this.setAttribute("position", new qr(U2.positions, 3)), this.setAttribute("normal", new qr(U2.normals, 3)), this.setAttribute("uv", new qr(U2.uvs, 2)), this._setIndexFromUint32(q);
      let s2 = D;
      s2.copy(u[u.length - 1]).setPosition(l[l.length - 1]), this.applyMatrix4OnRange(s2, B2, j2), s2.copy(u[0]).setPosition(l[0]).multiply(I.makeScale(1, 1, -1)), this.applyMatrix4OnRange(s2, B2 - O2, B2), this.reverseIndicesOnRange(e2, t2);
    } else this.setAttribute("position", new qr(U2.positions, 3)), this.setAttribute("normal", new qr(U2.normals, 3)), this.setAttribute("uv", new qr(U2.uvs, 2)), this._setIndexFromUint32(q);
  }
  ...
  _computeBasisMatrices(e) {
    ...
    for (let h2 = 0; h2 < s; h2++) {
      let u2, p2, c2 = e[h2];
      u2 = 0 === h2 ? i2 ? e[s - 1] : k.copy(c2).multiplyScalar(2).sub(e[1]) : e[h2 - 1], p2 = h2 === s - 1 ? i2 ? e[0] : k.copy(c2).multiplyScalar(2).sub(e[h2 - 1]) : e[h2 + 1], R.subVectors(c2, u2).normalize(), T.subVectors(p2, c2).normalize();
      let d2 = E.addVectors(R, T);
      d2.lengthSq() < 1e-10 && (d2.copy(T), d2.dot(r) < 0 && d2.negate()), d2.normalize(), r.copy(d2), 0 === h2 && (d2.equals(l) || z.copy(d2).negate().equals(l)) && l.set(0, 0, 1);
      let m = j.crossVectors(l, d2).normalize(), g = B.crossVectors(d2, m).normalize();
    ...
    h.dot(d) > 0 && (c *= -1);
    for (let e2 = 1; e2 < t.length; e2++) t[e2].multiply(O.makeRotationZ(c * e2 / t.length));
    return t;
  }
...
i("PathGeometry", (e, t, s) => new A(e, t, s)), v("PathGeometry", class {
  ...
  static normalizeInputs(e, t) {
    ...
    return { path: e.path ?? ur.defaultData(), parameters: Object.assign(s, { width: i2, height: n, depth: a2, extrusion: { ...vr, ...s.extrusion } }), shapeData: r };
  }
  static build(e) {
...
//# sourceMappingURL=runtime-paths-JMJGABOE-T6PXZDBB.js.map
