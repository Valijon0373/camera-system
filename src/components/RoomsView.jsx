...
import { Plus, Edit3, Trash2, Camera as CameraIcon, Check, X, Building, Info, SlidersHorizontal, RefreshCw, Eye, EyeOff, Search, Lock, ShieldAlert } from 'lucide-react';
import { FaPowerOff } from 'react-icons/fa6';
import { MdOutlineEdit } from 'react-icons/md';

export const RoomsView = () => {
  const { currentUser, rooms, cameras, addRoom, updateRoom, deleteRoom, searchQuery, setSearchQuery, showIpAddresses, setShowIpAddresses, theme, isCameraAllowedForUser, askConfirmation } = useApp();
  const isLight = theme === 'light';
  const isAdmin = currentUser?.role === 'admin';

  // Allowed cameras count for non-admin user
  const allowedCamerasForUser = cameras.filter(c => isCameraAllowedForUser(currentUser, c.id));
  
  // Modal states
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [infoCamera, setInfoCamera] = useState(null);
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-2 mt-0.5 font-mono text-xs">
              ...
              {!isAdmin && (
                <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${
                  allowedCamerasForUser.length > 0 ? 'bg-teal-400/15 border-teal-400/30 text-teal-400' : 'bg-rose-500/15 border-rose-500/30 text-rose-400'
                }`}>
                  🛡️ Siz uchun: {allowedCamerasForUser.length}/{cameras.length} ta ruxsat berilgan
                </span>
              )}
            </div>
          </div>
      ...
      {/* Warning for Operator with 0 allowed cameras */}
      {!isAdmin && allowedCamerasForUser.length === 0 && (
        <div className={`p-4 rounded-2xl border flex items-center gap-3 font-mono text-xs ${
          ...
          <span>
            Diqqat: Sizning hisobingizga hozircha birorta ham kamerani ko'rish huquqi (ruxsati) berilmagan. Iltimos, Administrator bilan bog'laning.
          </span>
        </div>
      )}
              ...
              {!isEditing && isAdmin && (
                <div className={`mt-3 pt-3 flex items-center justify-between border-t ${isLight ? 'border-slate-100' : 'border-white/5'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <MdOutlineEdit className="w-4 h-4 text-teal-400" />
                    </button>

                    <button
                      onClick={() => askConfirmation({
                        title: "Xonani o'chirish",
                        message: `Haqiqatdan ham "Xona № ${room.number} - ${room.name}"ni o'chirmoqchimisiz?`,
                        itemName: `Xona № ${room.number} (${room.name})`,
                    ...
                    <button
                      onClick={() => setInfoCamera({ camera: roomCamera || null, room })}
                      className={`p-2 rounded-xl border transition-all hover:scale-110 flex items-center justify-center cursor-pointer ${
                        isLight
                          ? 'bg-sky-50 border-sky-200 text-sky-600 hover:border-sky-400'
                          : 'bg-sky-500/15 border-sky-400/40 text-sky-300 hover:border-sky-400/70'
                      }`}
                      title="Kamera ma'lumotlari"
                    >
      ...
      {infoCamera && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#05080f]/80 backdrop-blur-sm" onClick={() => setInfoCamera(null)}>
          <div
            className={`relative w-full max-w-2xl glass-panel rounded-3xl p-6 shadow-2xl ${isLight ? 'border-slate-200' : 'border-white/10'}`}
          ...
          >
            <div className={`flex items-center justify-between pb-4 border-b mb-4 ${isLight ? 'border-slate-200' : 'border-white/10'}`}>
              <h3 className={`text-base font-bold flex items-center gap-2 ${isLight ? 'text-slate-900' : 'text-white'}`}>
                <Eye className={`w-4 h-4 ${isLight ? 'text-sky-600' : 'text-sky-300'}`} />
                Kamera ma'lumotlari
              </h3>
              <button
                onClick={() => setInfoCamera(null)}
            ...
            {infoCamera.camera ? (
              <div className="space-y-3 font-mono text-xs">
                {[
                  ['Nomi', infoCamera.camera.name],
                  ['Xona', `№ ${infoCamera.room.number} — ${infoCamera.room.name}`],
                  ['IP manzil', infoCamera.camera.ip || '—'],
                  ['Port', infoCamera.camera.port || '554'],
                  ['Protokol', infoCamera.camera.protocol || 'RTSP'],
                  ['Holat', infoCamera.camera.status || '—'],
                  ['Ping', infoCamera.camera.ping != null ? `${infoCamera.camera.ping} ms` : '—'],
                  ['RTSP / URL', infoCamera.camera.rtspUrl || '—'],
                  ['Yaratilgan', infoCamera.camera.createdAt || '—']
                ].map(([label, value]) => (
                  <div key={label} className={`rounded-xl px-3 py-2.5 border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-white/5 border-white/10'}`}>
                    <div className={`uppercase tracking-wide mb-0.5 ${isLight ? 'text-slate-500' : 'text-slate-500'}`}>{label}</div>
                    <div className={`break-all ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>{value}</div>
                  </div>
                ))}
              </div>
            ) : (
              <p className={`text-sm font-mono ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>
                Bu xonaga kamera biriktirilmagan.
              </p>
            )}
