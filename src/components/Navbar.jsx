...
import { Camera, LayoutDashboard, Grid, LogOut, User, Search, Shield, Activity, Sun, Moon, Eye, EyeOff, X } from 'lucide-react';
...
export const Navbar = () => {
  ...
  return (
    <header className={`sticky top-0 z-40 backdrop-blur-xl border-b transition-colors ${
      theme === 'light' ? 'bg-white/85 border-teal-100 text-slate-900 shadow-sm shadow-teal-900/5' : 'bg-[#070b14]/85 border-white/10 text-white shadow-lg'
    }`}>
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-3 md:gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-teal-400 via-sky-400 to-violet-400 p-0.5 shadow-lg shadow-teal-500/25 transition-transform hover:scale-105 cursor-pointer" onClick={() => setActiveTab('rooms')}>
              <div className={`w-full h-full rounded-[14px] flex items-center justify-center ${theme === 'light' ? 'bg-white' : 'bg-[#070b14]'}`}>
                <Camera className="w-6 h-6 text-teal-400 animate-pulse" />
              </div>
            </div>
            <div>
              <div className={`font-extrabold text-base tracking-tight flex items-center gap-1.5 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>
                UrSPI <span className="bg-gradient-to-r from-teal-400 via-sky-400 to-violet-400 bg-clip-text text-transparent">Camera System</span>
              ...
              <p className={`text-[11px] font-mono hidden sm:block ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'}`}>
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <button
              onClick={toggleTheme}
              className={`p-2.5 rounded-xl border transition-all flex items-center gap-2 text-xs font-mono font-semibold cursor-pointer ${
                theme === 'light'
            ...
            >
              {theme === 'light' ? (
                <>
                  <Sun className="w-4.5 h-4.5 text-amber-600 fill-amber-500" />
                  <span className="hidden sm:inline font-bold">Kunduzki</span>
                </>
              ) : (
                <>
                  <Moon className="w-4.5 h-4.5 text-cyan-400" />
                  <span className="hidden sm:inline font-bold">Tungi</span>
                </>
              )}
            </button>
            ...
            <div className={`flex items-center gap-2.5 px-3 py-1.5 rounded-xl border ${
              ...
              <div className="text-left hidden sm:block">
                <p className={`text-xs font-black leading-tight ${theme === 'light' ? 'text-slate-950' : 'text-slate-100'}`}>
                  {currentUser?.fullName || currentUser?.username}
                </p>
                <p className={`text-[10px] font-mono leading-tight font-black ${theme === 'light' ? 'text-black' : 'text-teal-300'}`}>
                  @{currentUser?.username} • <span className="uppercase font-bold">{currentUser?.role}</span>
                </p>
              </div>
            </div>
